...
for ($i = 1; $i -le $num_cases; $i++) {
    ...
    if ($elapsed_time -gt 2) {
        Write-Host "Test case $i : Failed (Timeout) (Time: $elapsed_time z)"
    } else {
        $matched = $true
        ...
        if ($matched) {
            Write-Host "Test case $i : Passed (Time: $elapsed_time z)"
            $passed_cases++
        } else {
            Write-Host "Test case $i : Failed (Time: $elapsed_time z)"
